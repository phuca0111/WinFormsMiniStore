...
def create_customer_by_phone(name, phone):
    ...
    customer_model = CustomerModel(db_path)
    try:
        customer_model.add_customer(name, phone, '', '', '', '')
...
def get_product_by_barcode(barcode):
    print(f"DEBUG: Đang tìm sản phẩm với barcode: {barcode}")
    db_path = get_db_path()
    ...
    if result:
        ...
        print(f"DEBUG: Tìm thấy sản phẩm: {ten_sanpham} - {ten_bienthe} - {dongia}")
        return {
    ...
    print(f"DEBUG: Không tìm thấy sản phẩm với barcode: {barcode}")
    return None

def add_to_cart(barcode, quantity):
    print(f"DEBUG: Đang thêm vào giỏ hàng: barcode={barcode}, quantity={quantity}")
    ...
    if product:
        for item in cart:
            if str(item.barcode).strip() == str(barcode).strip():
                ...
                print(f"DEBUG: Cập nhật số lượng sản phẩm trong giỏ hàng: {item.soluong}")
                return
        stt = len(cart) + 1
        ...
        print(f"DEBUG: Thêm sản phẩm mới vào giỏ hàng: {product['ten_sanpham']} - {product['ten_bienthe']}")
    else:
        print(f"DEBUG: Không thể thêm sản phẩm vào giỏ hàng vì không tìm thấy sản phẩm")

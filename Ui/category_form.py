...
def load_categories(tree):
    ...
    for cat in categories:
        tree.insert('', 'end', values=(cat.id, cat.ten))
...
def main():
    ...
    # Tạo Treeview để hiển thị danh sách thể loại
    columns = ('ID', 'Tên thể loại')
    tree = ttk.Treeview(root, columns=columns, show='headings', height=10)
    tree.heading('ID', text='ID')
    tree.heading('Tên thể loại', text='Tên thể loại')
    ...
    btn_refresh = tk.Button(frame_btn, text='Làm mới', width=10, command=lambda: load_categories(tree))

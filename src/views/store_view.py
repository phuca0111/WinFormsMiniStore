...
class StoreView:
    ...
    def create_widgets(self):
        ...
        label = ttk.Label(self.frame, text='Quản lý cửa hàng', font=("Arial", 18))
        label.grid(row=row_idx, column=0, columnspan=2, padx=20, pady=5, sticky=tk.N)
        row_idx += 1

        # Tạo Treeview để hiển thị danh sách cửa hàng
        self.tree = ttk.Treeview(self.frame, columns=("ID", "Tên cửa hàng", "Địa chỉ", "Số điện thoại", 
        ...
        self.tree.column("Ghi chú", width=300)
        ...
        self.tree.grid(row=row_idx, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        scrollbar.grid(row=row_idx, column=1, sticky=(tk.N, tk.S))
        row_idx += 1
        
        # Frame cho các nút
        button_frame = ttk.Frame(self.frame, padding="5")
        button_frame.grid(row=row_idx, column=0, columnspan=2, sticky=(tk.W, tk.E))
        ...
        # Tạo các nút
        ttk.Button(button_frame, text="Thêm mới", command=self.show_add_dialog).grid(row=0, column=0, padx=5, pady=5, sticky=tk.EW)
        ttk.Button(button_frame, text="Sửa", command=self.show_edit_dialog).grid(row=0, column=1, padx=5, pady=5, sticky=tk.EW)
        ttk.Button(button_frame, text="Xóa", command=self.delete_store).grid(row=0, column=2, padx=5, pady=5, sticky=tk.EW)
        ttk.Button(button_frame, text="Làm mới", command=self.refresh_data).grid(row=0, column=3, padx=5, pady=5, sticky=tk.EW)
        ttk.Button(button_frame, text="Chọn cửa hàng", command=self.select_store_dialog).grid(row=0, column=4, padx=5, pady=5, sticky=tk.EW)

...
class SupplierProductView(tk.Frame):
    ...
    def setup_ui(self):
        ...
        # Tạo frame chứa input và button (phần nhỏ)
        top_frame = tk.Frame(self, bg="#F5F7FA")
        top_frame.pack(fill=tk.X, padx=0, pady=0, expand=False)

        # Input nhỏ lại
        input_frame = tk.LabelFrame(top_frame, text="Thông tin nhập hàng", font=("Segoe UI", 12, "bold"),
                                   bg="#F5F7FA", fg="#222", bd=0)
        input_frame.pack(fill=tk.X, padx=32, pady=(16, 2), expand=False)
        tk.Label(input_frame, text="Nhà cung cấp:", font=("Segoe UI", 12), bg="#F5F7FA", fg="#222").grid(row=0, column=0, sticky="e", padx=8, pady=4)
        self.combo_supplier = ttk.Combobox(input_frame, font=("Segoe UI", 12), height=5)
        self.combo_supplier.grid(row=0, column=1, sticky="ew", padx=8, pady=4)
        tk.Label(input_frame, text="Biến thể sản phẩm:", font=("Segoe UI", 12), bg="#F5F7FA", fg="#222").grid(row=1, column=0, sticky="e", padx=8, pady=4)
        self.combo_variant = ttk.Combobox(input_frame, font=("Segoe UI", 12), height=5)
        self.combo_variant.grid(row=1, column=1, sticky="ew", padx=8, pady=4)
        tk.Label(input_frame, text="Số lượng nhập:", font=("Segoe UI", 12), bg="#F5F7FA", fg="#222").grid(row=2, column=0, sticky="e", padx=8, pady=4)
        self.entry_qty = ttk.Entry(input_frame, font=("Segoe UI", 12), width=10)
        self.entry_qty.grid(row=2, column=1, sticky="ew", padx=8, pady=4)
        tk.Label(input_frame, text="Giá nhập:", font=("Segoe UI", 12), bg="#F5F7FA", fg="#222").grid(row=3, column=0, sticky="e", padx=8, pady=4)
        self.entry_price = ttk.Entry(input_frame, font=("Segoe UI", 12), width=10)
        self.entry_price.grid(row=3, column=1, sticky="ew", padx=8, pady=4)
        tk.Label(input_frame, text="Hạn sử dụng (YYYY-MM-DD):", font=("Segoe UI", 12), bg="#F5F7FA", fg="#222").grid(row=4, column=0, sticky="e", padx=8, pady=4)
        self.entry_expiry = ttk.Entry(input_frame, font=("Segoe UI", 12), width=15)
        self.entry_expiry.grid(row=4, column=1, sticky="ew", padx=8, pady=4)
        input_frame.grid_columnconfigure(1, weight=1)

        # Nút chức năng pastel bo tròn, nhỏ lại
        btn_frame = tk.Frame(top_frame, bg="#F5F7FA")
        btn_frame.pack(fill=tk.X, padx=32, pady=(0, 8), expand=False)
        def style_btn(btn, color, hover):
            btn.configure(bg=color, fg="#222", activebackground=hover, activeforeground="#222", relief="flat", bd=0, font=("Segoe UI", 12, "bold"), cursor="hand2", padx=10, pady=6, highlightthickness=0, borderwidth=0)
            btn.bind("<Enter>", lambda e: btn.configure(bg=hover))
            btn.bind("<Leave>", lambda e: btn.configure(bg=color))
        btn_add = tk.Button(btn_frame, text='Thêm', command=self.add_supplier_product)
        style_btn(btn_add, "#eafaf1", "#d1f2eb")
        btn_add.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=6, pady=8)
        btn_update = tk.Button(btn_frame, text='Sửa', command=self.edit_supplier_product)
        style_btn(btn_update, "#f9e7cf", "#f6cba3")
        btn_update.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=6, pady=8)
        # btn_delete = tk.Button(btn_frame, text='Xóa', command=self.delete_supplier_product)
        # style_btn(btn_delete, "#fdeaea", "#f6bebe")
        # btn_delete.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=6, pady=8)
        btn_refresh = tk.Button(btn_frame, text='Làm mới', command=self.load_data)
        style_btn(btn_refresh, "#e8eaf6", "#c5cae9")
        btn_refresh.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=6, pady=8)

        # Bảng hiển thị dữ liệu: TO và chiếm phần lớn cửa sổ
        table_frame = tk.Frame(self, bg="#F5F7FA")
        table_frame.pack(fill=tk.BOTH, expand=True, padx=32, pady=(0, 24))
        ...
        self.tree = ttk.Treeview(table_inner, columns=columns, show='headings', height=20, style="Custom.Treeview")
